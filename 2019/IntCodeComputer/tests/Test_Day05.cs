...
{
    [Theory]
    [InlineData(1002,opCode.MULTIPLY,new bool[]{false,true})]
    [InlineData(10101,opCode.ADD,new bool[]{true,false,true})]
    public void CheckOpCode_And_ParameterModes(int instruction, opCode expectedOpCode, bool[] expectedParameterModes)
    {
        var myCpu = new intCode.Computer([]);
        (opCode opcode, bool[] paramModes) = myCpu.CheckOpCode(instruction);
        opcode.ShouldBe(expectedOpCode);
        paramModes.Length.ShouldBe(expectedParameterModes.Length);
        for(int i = 0; i < paramModes.Length; i++)
        {
            paramModes[i].ShouldBe(expectedParameterModes[i]);
        }
    }

    [Theory]
    [InlineData(1,opCode.ADD)]
    [InlineData(2,opCode.MULTIPLY)]
    [InlineData(3,opCode.INPUT)]
    [InlineData(4,opCode.OUTPUT)]
    public void CheckOpCodes_After_Adding_Input_And_Output_Support(int instruction, opCode expectedOpCode)
    {
        var myCpu = new intCode.Computer([]);
        (opCode opcode, bool[] paramModes) = myCpu.CheckOpCode(instruction);
        opcode.ShouldBe(expectedOpCode);
    ...
    [Theory]
    [InlineData(new int[]{3,0,4,0,99}, 66,0)]
    [InlineData(new int[]{3,1,4,0,99}, 66,1)]
    public void CheckInput(int[] program, int input, int expectedLocation)
    {
        var myCpu = new intCode.Computer(program);
        int result;
        myCpu.Input=input;
        bool status = myCpu.Run(out result);
        status.ShouldBeTrue();
        myCpu.Program[expectedLocation].ShouldBe(input);
    }



    [Theory]
    [InlineData(new int[]{3,0,4,0,99}, 0,0)]
    [InlineData(new int[]{3,0,4,1,99}, 66,0)]
    [InlineData(new int[]{3,8,4,8,99,0,3,6,24,77}, 0,0)]
    public void CheckInputAndOutput(int[] program, int input, int expectedOutput)
    {
        var myCpu = new intCode.Computer(program);
        int result;
        myCpu.Input=input;
