...
def setup(path):
    ...
    with open(path, "r") as f:
        ...
        return [
            tuple(int(value) for value in rangestring.split("-"))
            for rangestring in input
        ]


def part1(data):
    """
        finds the invalid IDs by looking for any ID which is made only of 
    ...
    for range_tuple in data:
        ...
        for number in seq:
            ...
            if len(string_of_value) % 2 != 0:
                continue
            if (
                string_of_value[: len(string_of_value) // 2]
                == string_of_value[len(string_of_value) // 2 :]
            ):
...
def part2(data):
    """
        Looking for invalid IDs by checking if it is made only of
